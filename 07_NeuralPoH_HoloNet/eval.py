...
p.add_argument('--root_path', type=str, default='./phases/temp_red_HOLONET_ASM', help='Directory where test phases are being stored.')
p.add_argument('--data_path', type=str, default='./data', help='Directory for the dataset')
p.add_argument('--recon_path', type=str, default='./recon', help='Directory for recon image')
...
recon_path = opt.recon_path

# Hyperparameters setting
cm, mm, um, nm = 1e-2, 1e-3, 1e-6, 1e-9
chan_strs = ('red', 'green', 'blue', 'rgb')
prop_dists = (20*cm, 20*cm, 20*cm)
wavelengths = (660 * nm, 532 * nm, 473 * nm)  # wavelength of each color
...
for k, target in enumerate(image_loader):
    ...
    for c in chs:
        ...
        # opt.scale_output = 0.95
        recon_lin_intensity = torch.sum(recon_amp_c ** 2 * 0.95, dim=1, keepdim=True)

        recon_amp_c = torch.pow(recon_lin_intensity, 0.5)
